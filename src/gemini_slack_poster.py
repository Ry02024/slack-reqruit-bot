...
class GeminiSlackPoster:
    ...
    def summary_client(self, original_text):
        summary_prompt = f"""
        以下の文章をもとに、求人情報の各案件を分かりやすく整理し、次のフォーマットで5件にまとめて出力してください。
        回答は必ずプレーンテキスト形式で、マークダウン記法や装飾は一切使用しないでください。

        ※ 注意：求人情報掲載サイト（例えば "Indeed", "求人ボックス", "障害者転職エージェント ハッピー", "スグJOB" など）の名称は出力結果に含めず、実際に求人を出している企業の正式名称のみを抽出してください。

        【期待する出力フォーマット例】
        --------------------------------------------------
        本日のデータサイエンス系障がい者求人はこちらです：
        
        1. 株式会社ブレインパッド - データサイエンティスト（ジュニアレベル）
            勤務地: 六本木一丁目駅直結（詳細不明）
            職種: データサイエンティスト
            給与: 記載なし
            勤務形態: 記載なし
        ...
        上記フォーマットと【要件】に従い、以下の【元のテキスト】に基づく求人情報要約を生成してください。
    ...
    def search_info(self, user_query):
        ...
        enhanced_query = (
            ...
            "また、もし出力結果が『複数の企業』となっている場合は、特に勤務地が「大阪府梅田本社オフィス」と示されている企業を代表例として特定し、その企業のみの情報を出力してください。"
        )
